...
router.post('/request', 
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (!patientId || !reason || !accessType || !urgencyLevel || !requestedDurationHours || !justification) {
        return res.status(400).json({
          success: false,
          error: 'Missing required fields: patientId, reason, accessType, urgencyLevel, requestedDurationHours, justification'
        } as ApiResponse);
      }
      ...
      if (!['medical_emergency', 'technical_support', 'data_recovery', 'audit_investigation'].includes(accessType)) {
        return res.status(400).json({
          success: false,
...
router.post('/:requestId/revoke',
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      await emergencyAccessCrypto.revokeEmergencyAccess(
        requestId,
        req.user?.userId!,
        reason
      );
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const canView = 
        ...
        (req.user?.role === 'clinic_admin' && req.user?.organizationId === emergencyAccess.organizationId);
        ...
        activeAccess.map(async (access) => {
          ...
          return {
            ...
            remainingHours: access.endTime ? 
              Math.max(0, Math.ceil((access.endTime.getTime() - Date.now()) / (1000 * 60 * 60))) : 0,
...
router.post('/validate',
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const isValid = emergencyAccess.isActive && 
      ...
      const response: ApiResponse = {
        ...
        data: {
          ...
          remainingHours: emergencyAccess.endTime ?
            Math.max(0, Math.ceil((emergencyAccess.endTime.getTime() - Date.now()) / (1000 * 60 * 60))) : 0,
          cryptographicKeysActive: encryptionKeys.length > 0,
          keyCount: encryptionKeys.length,
          message: isValid ? 'Emergency access token is valid with active crypto keys' : 'Emergency access token is invalid or expired',
          cryptographicallySecured: true
        }
      };
