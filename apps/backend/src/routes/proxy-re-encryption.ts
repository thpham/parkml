...
import { 
  ProxyReEncryption, 
  DelegationRequest 
} from '../crypto/proxy-re-encryption';
    ...
    try {
      ...
      if (!delegateeId || !dataCategories || !accessLevel || !validityDays || !reason) {
        return res.status(400).json({
          success: false,
          error: 'Missing required fields: delegateeId, dataCategories, accessLevel, validityDays, reason'
        } as ApiResponse);
      }
        ...
        delegations.map(async (delegation) => {
          ...
          return {
            ...
            remainingDays: Math.max(0, Math.ceil(
              (delegation.validUntil.getTime() - Date.now()) / (1000 * 60 * 60 * 24)
            )),
 ...
 */
router.post('/:delegationId/revoke',
  authenticateToken,
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      await ProxyReEncryption.revokeDelegation(
        delegationId,
        req.user?.userId!,
        reason
      );
 ...
 */
router.post('/:delegationId/re-encrypt',
  authenticateToken,
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { delegationId } = req.params;
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const canView = 
        ...
        (req.user?.role === 'clinic_admin' && req.user?.organizationId === delegation.organizationId);
