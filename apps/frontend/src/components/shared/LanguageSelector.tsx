...
export const LanguageSelector: React.FC<LanguageSelectorProps> = ({
          ...
          {supportedLanguages.map(lang => (
            <li key={lang.code}>
              ...
              >
                ...
                {language === lang.code && <div className="w-2 h-2 bg-primary rounded-full"></div>}
              </button>
            </li>
          ))}
        </ul>
        ...
        <div className="space-y-1">
          {supportedLanguages.map(lang => (
            ...
            >
              ...
              {language === lang.code && <div className="w-2 h-2 bg-current rounded-full"></div>}
            </button>
          ))}
        </div>
  ...
  return (
    <div className={`dropdown dropdown-end ${className}`}>
      <div tabIndex={0} role="button" className="btn btn-ghost normal-case flex items-center gap-2">
        <div className="flex items-center gap-2">
          <span className="text-lg">
            {supportedLanguages.find(lang => lang.code === language)?.flag || '🌐'}
          </span>
